/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import {
  IsString,
  MaxLength,
  IsOptional,
  ValidateNested,
  IsBoolean,
} from "class-validator";
import { EventManagerCreateNestedManyWithoutCitiesInput } from "./EventManagerCreateNestedManyWithoutCitiesInput";
import { Type } from "class-transformer";
import { EventCreateNestedManyWithoutCitiesInput } from "./EventCreateNestedManyWithoutCitiesInput";

@InputType()
class CityCreateInput {
  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @MaxLength(1000)
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  description?: string | null;

  @ApiProperty({
    required: false,
    type: () => EventManagerCreateNestedManyWithoutCitiesInput,
  })
  @ValidateNested()
  @Type(() => EventManagerCreateNestedManyWithoutCitiesInput)
  @IsOptional()
  @Field(() => EventManagerCreateNestedManyWithoutCitiesInput, {
    nullable: true,
  })
  eventManagers?: EventManagerCreateNestedManyWithoutCitiesInput;

  @ApiProperty({
    required: false,
    type: () => EventCreateNestedManyWithoutCitiesInput,
  })
  @ValidateNested()
  @Type(() => EventCreateNestedManyWithoutCitiesInput)
  @IsOptional()
  @Field(() => EventCreateNestedManyWithoutCitiesInput, {
    nullable: true,
  })
  events?: EventCreateNestedManyWithoutCitiesInput;

  @ApiProperty({
    required: false,
    type: Boolean,
  })
  @IsBoolean()
  @IsOptional()
  @Field(() => Boolean, {
    nullable: true,
  })
  isActive?: boolean | null;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @MaxLength(1000)
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  name?: string | null;
}

export { CityCreateInput as CityCreateInput };
