/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import {
  IsString,
  MaxLength,
  IsOptional,
  ValidateNested,
  IsBoolean,
} from "class-validator";
import { EventManagerUpdateManyWithoutCitiesInput } from "./EventManagerUpdateManyWithoutCitiesInput";
import { Type } from "class-transformer";
import { EventUpdateManyWithoutCitiesInput } from "./EventUpdateManyWithoutCitiesInput";

@InputType()
class CityUpdateInput {
  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @MaxLength(1000)
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  description?: string | null;

  @ApiProperty({
    required: false,
    type: () => EventManagerUpdateManyWithoutCitiesInput,
  })
  @ValidateNested()
  @Type(() => EventManagerUpdateManyWithoutCitiesInput)
  @IsOptional()
  @Field(() => EventManagerUpdateManyWithoutCitiesInput, {
    nullable: true,
  })
  eventManagers?: EventManagerUpdateManyWithoutCitiesInput;

  @ApiProperty({
    required: false,
    type: () => EventUpdateManyWithoutCitiesInput,
  })
  @ValidateNested()
  @Type(() => EventUpdateManyWithoutCitiesInput)
  @IsOptional()
  @Field(() => EventUpdateManyWithoutCitiesInput, {
    nullable: true,
  })
  events?: EventUpdateManyWithoutCitiesInput;

  @ApiProperty({
    required: false,
    type: Boolean,
  })
  @IsBoolean()
  @IsOptional()
  @Field(() => Boolean, {
    nullable: true,
  })
  isActive?: boolean | null;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @MaxLength(1000)
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  name?: string | null;
}

export { CityUpdateInput as CityUpdateInput };
